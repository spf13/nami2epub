...
tidy_options = {'output_xhtml' : 1, 'add_xml_decl':1, 'indent':1, 'tidy_mark':0, 'char_encoding': 'utf8'}
...
book_path=re.sub('[^a-zA-Z0-9\-_.() ]',' ',book_title)
book_path=book_path+'.'+re.sub('[^a-zA-Z0-9\-_.() ]',' ',book_author)+'.'+book_id
book_path0=book_path+''
n_path=0

if not os.path.exists(book_author):
    os.mkdir(book_author)

os.chdir(book_author)
